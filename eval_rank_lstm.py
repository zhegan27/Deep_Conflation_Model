'''
Training code for implementing Deep Conflation Model using LSTM encoder
'''
...
from model.lstm_matching import init_params, init_tparams, build_model, build_encoder
...
# trainer
def trainer(train, valid, test, n_chars=32,n_x=128,n_h=128,max_epochs=20,
            gamma=10,ncon=50,lrate=0.0002,batch_size=100,
            dispFreq=10,validFreq=100, saveto='example.npz'):
    
    """ train, valid, test : datasets
        n_chars : vocabulary size
        n_x : character embedding dimension
    ...
    valid_x, valid_x_mask = prepare_data(valid[0])
    valid_y, valid_y_mask = prepare_data(valid[1])
    
    test_x, test_x_mask = prepare_data(test[0])
    test_y, test_y_mask = prepare_data(test[1])
    ...
    for eidx in range(max_epochs):
        ...
        for minibatch in range(numbatches):
            ...
            x, x_mask = prepare_data(x)
            y, y_mask = prepare_data(y)
            cy, cy_mask = prepare_data(cy)
            
            cost = f_grad_shared(x, x_mask,y, y_mask,cy, cy_mask)
            f_update(lrate)
            ...
            if np.mod(uidx, validFreq) == 0:
                ...
                feats_x, feats_y = f_emb(valid_x, valid_x_mask, valid_y, valid_y_mask)
    ...
    feats_x, feats_y = f_emb(test_x, test_x_mask, test_y, test_y_mask)
    (r1, r3, r10, medr, meanr,h_meanr) = rank(feats_x, feats_y)
    logger.info('Test Rank:{}, {}, {}, {},{},{}'.format(r1, r3, r10, medr,meanr,h_meanr))
    
    # np.savez("./lstm_feats.npz", feats_x=feats_x, feats_y=feats_y)
...
if __name__=="__main__":  
    ...
    logger = logging.getLogger('eval_lstm_{}_{}'.format(data, query))
    logger.setLevel(logging.INFO)
    fh = logging.FileHandler('eval_lstm_{}_{}.log'.format(data, query))
    ...
    saveto = "lstm_results_{}_{}".format(data, query)
    ...
    if data == "predefined_split":
        ...
        np.savez("./lstm_ten_fold_{}_{}.npz".format(data, query), results=results)
